      ...
      child: BlocListener<VoteCubit, VoteState>(
        ...
        listener: (context, state) {
          ...
          if (state.status == VoteStatus.failure) {
            EventTrigger.error(
              context: context,
              msg: 'Sorry that has not worked out. Try again.',
            );
          }
        },
        child: BlocSelector<MembersBloc, MembersState, bool>(
          selector: (state) =>
              MembersSelector.canVote(state, ranking.identityId),
          builder: (context, canVote) {
            return canVote
                ...
                : BlocSelector<MembersBloc, MembersState, bool>(
                    selector: (state) => MembersSelector.canRevokeVote(
                        state, ranking.identityId),
                                  ...
                                  builder: (context, state) {
                                    return BlocBuilder<VoteCubit, VoteState>(
                                      builder: (context, state) {
                                        return AnimatedOpacity(
                                          opacity:
                                              state.status == VoteStatus.loading
                                                  ? 0.0
                                                  : 1.0,
                                          duration:
